...
int main() {
    ...
    //暗号化メッセージファイルciphertext3.datを読み込み、鍵ファイルbob.keyを読み込み、暗号化メッセージを復号し、復号したデータをファイル名dextext_bob.datとして出力
    std::string plaintext;
    std::string ciphertext;
    std::string key;
    std::string encrypted_filename = "ciphertext3.dat";
    std::string key_filename ="bob.key";
    std::string decrypted_filename = "dectext_bob.dat";
